...
class UserController {
    async create(req, res) {
        ...
        const checkUserExists = await (0, knex_1.knexCon)("users").where({ email }).first();
        if (checkUserExists) {
            throw new AppError_1.AppError("Este e-mail já está em uso.");
        }
        const hashedPassword = await (0, bcryptjs_1.hash)(password, 8);
        await (0, knex_1.knexCon)("users").insert({ name, email, password: hashedPassword });
    ...
    async update(req, res) {
        ...
        if (admin)
            throw new AppError_1.AppError("Usuários não podem alterar administradores!", 401);
        let user = await (0, knex_1.knexCon)("users").where({ id }).first();
        if (!user) {
            throw new AppError_1.AppError("Usuário não encontrado.");
        }
        const userWithUpdatedEmail = await (0, knex_1.knexCon)("users").where({ email }).first();
        ...
        await (0, knex_1.knexCon)("users").where({ id }).update(user);
